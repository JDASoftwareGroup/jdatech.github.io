...
- `const_0_1_2`: This are the inner cells of the cube. There are `length^3` unique values, as many as there are rows in
  the DataFrame.
...
With growing edge size of the cube, the number of rows grows polynomial:
