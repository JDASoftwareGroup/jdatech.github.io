...
`city` column, we can see that the same entry is multiple times before a new city starts. Instead storing each entry, it
would for sure be smarter to just store a city together with a number of times it is repeated, e.g. "Berlin - 1000
times, Hamburg - 500 times, ...". This is what [Run-length encoding](https://en.wikipedia.org/wiki/Run-length_encoding)
does. It can compress that data without precision-loss (in our benchmark **over 200x memory reduction**) while at the same
time it can also be faster then uncompressed data, in our measurements **over 50 times faster**.
...
Since pandas requires us to be able to do quick [random access](https://en.wikipedia.org/wiki/Random_access), e.g. for
sorting and group-by operation, instead of the actual run-lengths we store the end positions of each run (this is the
...
rle-array are implemented using [Numba](https://numba.pydata.org/). For operations like group-by however the current
...
Summarizing this we can say, the performance depends on the length of the runs (longer runs give better compression and
performance) and that rle-array works great simple operations but has issues with
...
suited for the concrete use case. Here is a short illustration about a sub-selection of techniques and readers are
...
### Data Types
Pandas lets you not only choose the semantics of a data type (e.g. integer, floats, booleans) but in some cases also the
size of it:
...
The advantage is that you can safe even more memory, but in return you can pay with heavy performance penalties, since
...
decompress the run-length encoded data and perform the operation mostly on an NumPy array. While this is functional
correct, we would like to stay in the compressed format and implement our own specialized version of it. This will be
done step-by-step in future releases.
