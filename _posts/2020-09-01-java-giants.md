...
A fourth giant related to this discussion is **Chris Newland** who created the visualization tool **JITWatch** for you to see your code, the compiled bytecode, and the JIT assembler all in one glorious screen.   With this tool you could verify that, after running a higher volume performance run of your code, the resulting assembler does indeed contain an infinite loop even though the original java code seen beside it does not read as such (at first glance).   Enjoy my favorite presentation of his on his tool called [Understanding HotSpot JVM Performance with JITWatch](http://vimeo.com/181925278).

As a quick example, and an excuse to play with JMH and JITWatch, suppose we have a JMH benchmark with the following:
```java
    @Param("525600")
    long answer;
...
The assembly version above is the output of the C1 JIT compiler, which does fast simple-stupid when possible.   The assembly for called method is equally straightforward so I'll skip showing it, but even the pointless additions and subtractions are included.  
...
Note the bytecode is unchanged.   So, again, if you try to debug the new optimized code of "get()", you'll step through the (unchanged) bytecode and fail to reproduce the exact same behavior.  
